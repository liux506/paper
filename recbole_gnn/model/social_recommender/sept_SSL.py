...
class CrossTransformerSocial(nn.Module):
...
class MERG(nn.Module):
    ...
    def forward(self, g, h, e, s_u):
        ...
        hs = g.ndata['emb_h']
        edge = self.CrossT(s_u, g).squeeze(0)
...
class MERGSocial(nn.Module):
    ...
    def __init__(self, in_dim, hidden_dim, max_node_num, global_layer_num=2, dropout=0.1):
        ...
        self.bn_node_lr_e = nn.BatchNorm1d(hidden_dim)
        self.max_node_num = max_node_num
        self.CrossTSocial = CrossTransformerSocial(hidden_dim, nhead=1, layer_nums=1, attention_type='linear')
    ...
    def forward(self, g, h, e, s_u):
        ...
        hs = g.ndata['emb_h']
        edge = self.CrossTSocial(s_u, g).squeeze(0)
...
class SEPT(SocialRecommender):
    ...
    def get_user_view_matrix(self, dataset):
        ...
        social_mat = dataset.net_matrix()   #  (n,n)
        ...
        self.s_g = dgl.from_scipy(social_matrix, device=self.device)
        self.s_e = torch.nn.Embedding(num_embeddings=social_edge_index.size(1), embedding_dim=self.latent_dim)
    ...
    def forward(self, graph=None):
        ...
        e = self.merg(self.g, all_embeddings, e, s_embeddings)
    ...
    def forward(self, graph=None):
        ...
        for gnn in self.layers:
            ...
            social_embeddings, socialedge_embeddings = gnn(self.s_g, social_embeddings, s_e)
            normS_embeddings = F.normalize(social_embeddings, p=2, dim=1)
            embeddingsS_list.append(normS_embeddings)
    ...
    def calculate_rec_loss(self, interaction):
        ...
        '''
        # user SSL
        u_emd1 = F.normalize(self.user_all_embeddings[user], dim=1)
        u_emd2 = F.normalize(social_embeddings[user], dim=1)
        all_user2 = F.normalize(social_embeddings, dim=1)
    ...
    def predict(self, interaction):
        ...
        user_all_embeddings, item_all_embeddings, social = self.forward()
    ...
    def full_sort_predict(self, interaction):
        ...
        if self.restore_user_e is None or self.restore_item_e is None:
            self.restore_user_e, self.restore_item_e, self.social = self.forward()
