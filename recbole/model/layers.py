...
class MLPLayers(nn.Module):
    ...
    Args:
        - layer(list): a list contains the size of each layer in mlp layer
        - dropout(float): probability of an element to be zeroed. Default: 0
        - activation(str): activation function after each layer in mlp layer. Default: 'relu'.
                           candidates: 'sigmoid', 'tanh', 'relu', 'leekyrelu', 'none'

    Shape:

        - Input: (:math:`N`, \*, :math:`H_{in}`) where \* means any number of additional dimensions
          :math:`H_{in}` must equal to the first value in `layer`
        - Output: (:math:`N`, \*, :math:`H_{out}`) where :math:`H_{out}` equals to the last value in `layer`
...
class MultiHeadAttention(nn.Module):
    """
    Multi-head Self-attention layer, a attention score dropout layer is introduced.
    ...
    def forward(self, input_tensor, attention_mask):
        ...
        # Apply the attention mask is (precomputed for all layer in BertModel forward() function)
...
class FeedForward(nn.Module):
    """
    Point-wise feed-forward layer is implemented by two dense layer.
...
class TransformerEncoder(nn.Module):
    ...
    Args:
        n_layers(num): num of transformer layer in transformer encoder. Default: 2
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True):
        ...
        Args:
            ...
            output_all_encoded_layers (Bool): whether output all transformer layer' output

        Returns:
            all_encoder_layers (list): if output_all_encoded_layers is True, return a list consists of all transformer
            layer' output, otherwise return a list only consists of the output of last transformer layer.
...
class LightTransformerEncoder(nn.Module):
    ...
    def forward(self, hidden_states, pos_emb, output_all_encoded_layers=True):
        ...
        Returns:
            all_encoder_layers (list): if output_all_encoded_layers is True, return a list consists of all transformer layer' output,
...
class CNNLayers(nn.Module):
    ...
    Args:
        - channels(list): a list contains the channels of each layer in cnn layer
        - kernel(list): a list contains the kernels of each layer in cnn layer
        - strides(list): a list contains the channels of each layer in cnn layer
        - activation(str): activation function after each layer in mlp layer. Default: 'relu'
                      candidates: 'sigmoid', 'tanh', 'relu', 'leekyrelu', 'none'
